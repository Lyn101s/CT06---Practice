...
function draw() {
  ...
  //align group of circles to center
  // let circleDia = 30;
  // let numOfCircle = 5;
  ...
  //   circle( startX + i*circleDia , height/2, circleDia);
  ...
  // for(let i=0; i<numOfCircle; i++) {
  //   circle( startX + i*circleDia , circleDia/2, circleDia);
...
//align group of circles to the left
  ...
  let numOfCircle = 5;
  let totalDia = numOfCircle*circleDia;
  let startX = circleDia/2; // only start with radius
  let startY = circleDia/2;

  for(let j=0; j<numOfCircle; j++) {
    let y = startY + j*circleDia; //change to different height after inner loop is completed; j*circlDia - 1 circle away, 2 circles away
    for(let i=0; i<numOfCircle; i++) {
